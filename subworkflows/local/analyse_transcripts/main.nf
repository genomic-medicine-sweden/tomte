...
workflow ANALYSE_TRANSCRIPTS {
    ...
    emit:
        transcript_gtf        = !skip_stringtie ? STRINGTIE_STRINGTIE.out.transcript_gtf
                                                : Channel.empty()   // channel: [ val(meta), [ path(transctript_gtf)] ]
        abundance             = !skip_stringtie ? STRINGTIE_STRINGTIE.out.abundance
                                                : Channel.empty()   // channel: [ val(meta), [ path(abundance) ] ]
        coverage_gtf          = !skip_stringtie ? STRINGTIE_STRINGTIE.out.coverage_gtf
                                                : Channel.empty()   // channel: [ val(meta), [ path(coverage_gtf) ] ]
        annotated_gtf         = !skip_stringtie ? GFFCOMPARE.out.annotated_gtf
                                                : Channel.empty()   // channel: [ val(meta), [ path(annotated_gtf) ] ]
        stats_gtf             = !skip_stringtie ? GFFCOMPARE.out.stats
                                                : Channel.empty()   // channel: [ val(meta), [ path(stats) ] ]
        annotation_drop       = ( !skip_drop_ae | !skip_drop_as ) ? DROP_SAMPLE_ANNOT.out.drop_annot
                                                : Channel.empty()   // channel: [ path(sample_annotation.tsv) ]
        config_drop_ae        = !skip_drop_ae ? DROP_CONFIG_RUN_AE.out.config_drop
                                                : Channel.empty()   // channel: [ path(confg_file.yml) ]
        drop_ae_out           = !skip_drop_ae ? DROP_CONFIG_RUN_AE.out.drop_ae_out
                                                : Channel.empty()   // channel: [ path(drop_output_AE) ]
        config_drop_as        = !skip_drop_as ? DROP_CONFIG_RUN_AS.out.config_drop
                                                : Channel.empty()   // channel: [ path(confg_file.yml) ]
        drop_as_out           = !skip_drop_as ? DROP_CONFIG_RUN_AS.out.drop_as_out
                                                : Channel.empty()   // channel: [ path(drop_output_AS) ]
        drop_ae_out_clinical  = !skip_drop_ae ? DROP_FILTER_RESULTS.out.ae_out_clinical
                                                : Channel.empty()   // channel: [ path(drop_AE_clinical.tsv) ]
        drop_ae_out_research  = !skip_drop_ae ? DROP_FILTER_RESULTS.out.ae_out_research
                                                : Channel.empty()   // channel: [ path(drop_AE_research.tsv) ]
        drop_as_out_clinical  = !skip_drop_as ? DROP_FILTER_RESULTS.out.as_out_clinical
                                                : Channel.empty()   // channel: [ path(drop_AS_clinical.tsv) ]
        drop_as_out_research  = !skip_drop_as ? DROP_FILTER_RESULTS.out.as_out_research
                                                : Channel.empty()   // channel: [ path(drop_AS_research.tsv) ]
