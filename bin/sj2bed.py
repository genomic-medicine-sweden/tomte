    ...
    with open(sj) as file:
        ...
        for line in file:
            ...
            if elem[6] == 0:
                ...
                new_line = str(elem[0]) + "\t" + \
                    str(int(elem[1])-1) + "\t" + str(elem[2]) + "\t"
                my_names = ["motif="+str(motifs[int(elem[4])]),
                ...
                new_line = new_line+"\t"+";".join(my_names).replace(
                    '\n', "")+"\t"+str(elem[6])+"\t"+str(strands[int(elem[3])])+'\n'
                res.write(new_line)
    res.close()
    file.close()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--sj", type=str, help="SJ.out.tab file generated by STAR", required=True)
    parser.add_argument("--output", type=str,
                        help="output bed file name", required=True)
