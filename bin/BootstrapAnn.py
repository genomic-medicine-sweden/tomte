...
parser = argparse.ArgumentParser(
    """Add bootstrap and binomial P values to a vcf file, the output vcf is printed to stdout"""
)
...
for line in open(args.ase):
    ...
    ase_list[content[0]][content[1]][content[4]] = {
        "ref_count": int(content[5]),
        "alt_count": int(content[6]),
        "tot_count": int(content[7]),
        "p_bin": p_bin,
        "non_param": -1,
    }
    ...
    if not int(content[7]):
        continue
    if (
        int(content[6]) / float(content[7]) < args.r
        or int(content[6]) / float(content[7]) > args.R
        or args.m > int(content[7])
    ):
        continue
...
for line in reader(args.vcf):
    ...
    if line[0] == "#":
        if not line[1] == "#":
            print(
                '##FORMAT=<ID=BT,Number=4,Type=Float,Description="BootstrapAnn p-values and GATK-ASEcounter stats (alt_count,total_count,binomial,nonparametric(-1 if not tested) )">'
            )
        print(line.strip())
        continue
    ...
    if content[0] in ase_list:
        if content[1] in ase_list[content[0]]:
            if content[4] in ase_list[content[0]][content[1]]:
                ...
                content[9] += ":{},{},{},{}".format(
                    ase_list[content[0]][content[1]][content[4]]["alt_count"],
                    ase_list[content[0]][content[1]][content[4]]["tot_count"],
                    bin_p,
                    non_param_p,
                )
                print("\t".join(content))
