...
def FilteringResults(
    samples: list, gene_panel: str, out_drop_ae_rds: str, out_drop_gene_name: str, out_drop_as_tsv: str
):
    ...
    if out_drop_ae_rds != "None":
        ...
        for id in samples:
            ...
            if sum(df_id["aberrant"] == "True") < 20:
                ...
                df_family_AE_top_hits = pd.concat([df_family_AE_top_hits, df_id[0:20]], ignore_index=True, sort=False)
            else:
                df_family_AE_top_hits = pd.concat([df_family_AE_top_hits, df_id], ignore_index=True, sort=False)
        ...
        df_family_annotated_AE_top_hits = df_genes.merge(df_family_AE_top_hits, left_on="gene_id", right_on="geneID")
        df_family_annotated_AE_top_hits.to_csv(
            "OUTRIDER_provided_samples_top_hits.tsv", sep="\t", index=False, header=True
        )

        # Only genes from panel
        if gene_panel != "None":
            df_clinical_AE = df_family_annotated_AE_top_hits.loc[
                df_family_annotated_AE_top_hits["hgncSymbol"].isin(df_panel["hgnc_symbol"])
            ]
            df_clinical_AE = df_panel[["hgnc_symbol", "hgnc_id"]].merge(
                df_family_annotated_AE_top_hits, left_on="hgnc_symbol", right_on="hgncSymbol"
            )
            df_clinical_AE = df_clinical_AE.drop(columns=["hgnc_symbol"])
            df_clinical_AE.to_csv("OUTRIDER_provided_samples_top_hits_filtered.tsv", sep="\t", index=False, header=True)
    ...
    if out_drop_as_tsv != "None":
        ...
        if gene_panel != "None":
            ...
            df_clinical_AS.to_csv("FRASER_provided_samples_top_hits_filtered.tsv", sep="\t", index=False, header=True)
